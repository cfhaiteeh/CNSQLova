...
def load_wikisql(path_wikisql, toy_model, toy_size, bert=False, no_w2i=False, no_hs_tok=False, aug=False):
    # Get data
    train_data, train_table = load_wikisql_data(path_wikisql, mode='train/train', toy_model=toy_model, toy_size=toy_size,
                                                no_hs_tok=no_hs_tok, aug=aug)
    val_data, val_table = load_wikisql_data(path_wikisql, mode='val/val', toy_model=toy_model, toy_size=toy_size,
                                            no_hs_tok=no_hs_tok)
    ...
    return train_data, train_table, val_data, val_table, w2i, wemb
...
def get_fields_2(t1, tables, no_hs_t=False, no_sql_t=False):
    nlu1 = t1['question']
    # nlu_t1 = t1['question_tok']
    nlu_t1 = [char for char in t1['question']]
    tid1 = t1['table_id']
    ...
    return nlu1, nlu_t1, tid1, None, None, sql_t1, tb1, hs_t1, hs1
...
def get_fields(t1s, tables, no_hs_t=False, no_sql_t=False, result=False):
    nlu, nlu_t, tid, sql_i, sql_q, sql_t, tb, hs_t, hs = [], [], [], [], [], [], [], [], []
    for t1 in t1s:
        if no_hs_t:
            if result:
                nlu1, nlu_t1, tid1, sql_i1, sql_q1, sql_t1, tb1, hs_t1, hs1 = get_fields_2(t1, tables, no_hs_t,
                                                                                           no_sql_t)
            else:
                nlu1, nlu_t1, tid1, sql_i1, sql_q1, sql_t1, tb1, hs_t1, hs1 = get_fields_1(t1, tables, no_hs_t,
                                                                                           no_sql_t)
        else:
            if result:
                nlu1, nlu_t1, tid1, sql_i1, sql_q1, sql_t1, tb1, hs_t1, hs1 = get_fields_2(t1, tables, no_hs_t,
...
def get_g(sql_i):
    ...
    for b, psql_i1 in enumerate(sql_i):
        ...
        g_sc.append(sorted(psql_i1["sel"]))
        g_sa.append(psql_i1["agg"])
        g_wco.append(psql_i1['cond_conn_op'])
        conds = psql_i1['conds']
        if not psql_i1["agg"][0] < 0:
    ...
    return g_sn, g_sc, g_sa, g_wn, g_wc, g_wo, g_wv, g_wco


def get_g_wvi_corenlp(t, q, gt_cond_seq):
    g_wvi_corenlp = []
    # for t1 in t:
    ...
    for cur_q, ans in zip(q, gt_cond_seq):
        ...
        for idx, item in enumerate(record):
            ...
            if item[0]:
                temp_ret_seq.append([temp_q.index(item[1]), temp_q.index(item[1]) + len(item[1]) - 1])
            else:
                temp_ret_seq.append([0, len(item[1]) - 1])
        g_wvi_corenlp.append(temp_ret_seq)
    # print 'retseq',ret_seq
...
def get_bert_output(model_bert, tokenizer, nlu_t, hds, max_seq_length):
    ...
    for b, nlu_t1 in enumerate(nlu_t):
        ...
        while len(input_ids1) < max_seq_length:
            ...
            input_mask1.append(0)
            segment_ids1.append(0)
        # with open('errout','a') as f:
        #   print(len(input_ids1) ,max_seq_length,file=f)
        #   print(u''.join(tokens1),file=f)
        #   print('xxxxxx',file=f)
...
def pred_sn(s_sn):
    ...
    pr_sn = []
    for s_sn1 in s_sn:
        pr_sn.append(s_sn1.argmax().item())
        # print(pr_wn, s_wn1)
...
def pred_dwsc(s_sc):
    ...
    for s_sc1 in s_sc:
        re2 = map(s_sc1.tolist().index, heapq.nlargest(2, s_sc1.tolist()))  # 求最大的三个索引    nsmallest与nlargest相反，求最小
        dwpr_sc.append(list(re2))
    return dwpr_sc
...
def pred_wc(wn, s_wc, typ=False, p_wn=None):
    """
    return: [ pr_wc1_i, pr_wc2_i, ...]
    ...
    for b, wn1 in enumerate(wn):
        s_wc1 = s_wc[b]
        pr_wc1 = argsort(-s_wc1.data.cpu().numpy())[:wn1]
        scorewc = sort((-s_wc1.data.cpu().numpy() * 100000).astype(np.int))
        _ans = []
        _id = list(pr_wc1)
        _val = list(scorewc)
        for i in range(0, wn1):
            if (_val[i] < 400000):
                _ans.append(_id[i])
            else:
                _ans.append(_id[0])
...
def pred_wc_sorted_by_prob(s_wc):
    ...
    dwpr_sc = []
    pr_wc = []
    for b, s_wc1 in enumerate(s_wc):
        re2 = map(s_wc1.tolist().index, heapq.nlargest(wn[b], s_wc1.tolist()))  # 求最大的三个索引    nsmallest与nlargest相反，求最小
        x = list(re2)
        dwpr_sc.append(sorted(x))
        pr_wc.append(sorted(x))
    return pr_wc
...
def pred_wco(s_wco):
    ...
    pr_wco = []
    for s_wco1 in s_wco:
        pr_wco.append(s_wco1.argmax().item())
        # print(pr_wn, s_wn1)
        # if s_wn1.argmax().item() == 3:
        #     input('')
...
def pred_sw_se(s_sn, s_sc, s_sa, s_wn, s_wc, s_wo, s_wv, s_wco, typ=False):
    pr_sn = pred_sn(s_sn)
    dwpr_sc = pred_dwsc(s_sc)
    pr_sc = pred_sc(pr_sn, s_sc)
    pr_sa = pred_sa(pr_sn, s_sa)
    pr_wn = pred_wn(s_wn)
    pr_wc = pred_wc(pr_wn, s_wc, typ, pr_wn)
    pr_wo = pred_wo(pr_wn, s_wo)
    pr_wvi = pred_wvi_se(pr_wn, s_wv)
    pr_wco = pred_wco(s_wco)

    return pr_sn, pr_sc, pr_sa, pr_wn, pr_wc, pr_wo, pr_wvi, dwpr_sc, pr_wco
...
def get_cnt_wv_list(g_wn, g_wc, g_sql_i, pr_sql_i, mode):
    ...
    for i in range(len(g_wn)):
        g_c.append(g_sql_i[i]['conds'])
        p_c.append(pr_sql_i[i]['conds'])

    for i in range(len(g_wn)):
        g_one = g_c[i]
        p_one = p_c[i]
        if len(g_one) != len(p_one):
            cnt_list.append(0)
        else:
            g_one = sorted(g_one, key=lambda l: l[0], reverse=False)
            p_one = sorted(p_one, key=lambda l: l[0], reverse=False)
            # print(g_one)
            Flag = True
            for j in range(len(g_one)):
                if g_one[j][2] != p_one[j][2]:
                    Flag = False
                    break
            if Flag:
                cnt_list.append(1)
...
def get_cnt_sw_list(g_sn, g_sc, g_sa, g_wn, g_wc, g_wo, g_wvi, g_wco, pr_sn,
                    pr_sc, pr_sa, pr_wn, pr_wc, pr_wo, pr_wvi, pr_wco,
                    g_sql_i, pr_sql_i,
                    mode):
    """ usalbe only when g_wc was used to find pr_wv
    """
    ...
    cnt_wco = get_cnt_sc_list(g_wco, pr_wco)

    return cnt_sn, cnt_sc, cnt_sa, cnt_wn, cnt_wc, cnt_wo, cnt_wvi, cnt_wv, cnt_wco


def get_cnt_lx_list(cnt_sn1, cnt_sc1, cnt_sa1, cnt_wn1, cnt_wc1, cnt_wo1, cnt_wv1, cnt_wco1):
    # all cnt are list here.
    cnt_list = []
    cnt_lx = 0
    for csn, csc, csa, cwn, cwc, cwo, cwv, cwco in zip(cnt_sn1, cnt_sc1, cnt_sa1, cnt_wn1, cnt_wc1, cnt_wo1, cnt_wv1,
                                                       cnt_wco1):
        if csn and csc and csa and cwn and cwc and cwo and cwv and cwco:
            cnt_list.append(1)
        else:
            cnt_list.append(0)
...
def get_cnt_x_list(engine, tb, g_sc, g_sa, g_wco, g_sql_i, pr_sc, pr_sa, pr_wco, pr_sql_i):
    cnt_x1_list = []
    g_ans = []
    pr_ans = []
    for b in range(len(g_sc)):
        g_ans1 = engine.execute(tb[b]['id'], g_sc[b], g_sa[b], g_sql_i[b]['conds'], g_wco[b])
        # print(f'cnt: {cnt}')
        # print(f"pr_sql_i: {pr_sql_i[b]['conds']}")
        try:
            pr_ans1 = engine.execute(tb[b]['id'], pr_sc[b], pr_sa[b], pr_sql_i[b]['conds'], pr_wco[b])
...
def generate_sql_i(pr_sn, pr_sc, pr_sa, pr_wn, pr_wc, pr_wo, pr_wv_str, pr_wco, nlu):
    pr_sql_i = []
    for b, nlu1 in enumerate(nlu):
        conds = []
        # print(pr_wc[b],pr_wn[b])
        for i_wn in range(pr_wn[b]):
            ...
            conds.append(conds1)
        # print("########")
        pr_sql_i1 = {'agg': pr_sa[b], 'sel': pr_sc[b], 'cond_conn_op': pr_wco[b], 'conds': conds}
...
def save_for_evaluation(path_save, results, dset_name, ):
    ...
    if dset_name=='test':
        results.insert(1882,results[1])
        results.insert(1883,results[1])
        results.insert(1884,results[1])
...
def generate_sql_q1(sql_i1, tb1):
    ...
    select_table = tb1["id"]
    if isinstance(sql_i1['agg'], list) == False:
        sql_i1['agg'] = [sql_i1['agg']]
    if isinstance(sql_i1['sel'], list) == False:
        sql_i1['sel'] = [sql_i1['sel']]
    select_agg = agg_ops[sql_i1['agg'][0]]
    select_header = headers[sql_i1['sel'][0]]
