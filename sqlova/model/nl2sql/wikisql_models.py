...
class Seq2SQL_v1(nn.Module):
    def __init__(self, iS, hS, lS, dr, n_cond_ops, n_agg_ops,n_conn_sql_dict, old=False):
        ...
        self.max_wn = 4
        self.n_cond_ops = n_cond_ops#4
        self.n_agg_ops = n_agg_ops#6

        self.snp=SelNumPredictor(iS, hS, lS, dr)
        self.scp = SCP(iS, hS, lS, dr)
        self.sap = SAP(iS, hS, lS, dr, n_agg_ops, old=old)

        self.wco = WCO(iS, hS, lS, dr, n_conn_sql_dict, old=old)
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs,
                g_sn=None,g_sc=None, g_sa=None, g_wn=None, g_wc=None, g_wo=None, g_wvi=None,g_wco=None,
                show_p_sc=False, show_p_sa=False,
                show_p_wn=False, show_p_wc=False, show_p_wo=False, show_p_wv=False):

        # ns
        s_sn=self.snp(wemb_n, l_n, wemb_hpu, l_hpu, l_hs)
        ...
        # sa
        s_sa = self.sap(wemb_n, l_n, wemb_hpu, l_hpu, l_hs, sn=pr_sn,sc=pr_sc, show_p_sa=show_p_sa)
        if g_sa:
        ...
        s_wco=self.wco(wemb_n, l_n, wemb_hpu, l_hpu, l_hs)
        if g_wco:
            pr_wco = g_wco
        else:
            pr_wco = pred_wco(s_wco)

        return s_sn,s_sc, s_sa, s_wn, s_wc, s_wo, s_wv,s_wco
...
class SelNumPredictor(nn.Module):
    def __init__(self, iS=300, hS=100, lS=2, dr=0.3):
        ...
        self.S_att_h = nn.Linear(hS, 1)
        self.S_hidden = nn.Linear(hS, lS * hS)
        self.S_cell = nn.Linear(hS, lS * hS)

        self.S_att_n = nn.Linear(hS, 1)
        self.sn_out = nn.Sequential(nn.Linear(hS, hS),
                                    nn.Tanh(),
                                    nn.Linear(hS, self.mL_S + 1))  # max number (2 + 1)
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs):
        ...
        att_h = self.S_att_h(wenc_hs).squeeze(2)
        ...
        att_n = self.S_att_n(wenc_n).squeeze(2)  # [B, max_len, 100] -> [B, max_len, 1] -> [B, max_len]
...
class SAP(nn.Module):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs,sn,sc, show_p_sa=False):
        # Encode
        ...
        for b in range(bS):
            ...
            pad = (self.mL_s - sn[b]) * [wenc_hs[b, 0]]  # this padding could be wrong. Test with zero padding later.
...
class WCO(nn.Module):
    def __init__(self, iS=300, hS=100, lS=2, dr=0.3, n_conn_sql_dict=-1, old=False):
        super(WCO, self).__init__()
        self.iS = iS
        self.hS = hS
        self.lS = lS
        self.dr = dr
        self.mL_C = n_conn_sql_dict-1  # max index
        ...
        self.S_att_n = nn.Linear(hS, 1)
        self.wco_out = nn.Sequential(nn.Linear(hS, hS),
                                    nn.Tanh(),
                                    nn.Linear(hS, self.mL_C + 1))  # max number (2 + 1)
...
def Loss_sw_se(s_sn,s_sc, s_sa, s_wn, s_wc, s_wo, s_wv,s_wco,g_sn, g_sc, g_sa, g_wn, g_wc, g_wo, g_wvi,g_wco):
    """
...
def Loss_wv_se(s_wv, g_wn, g_wvi):
    ...
    return loss
def Loss_wco(s_wco,g_wco):
    loss = F.cross_entropy(s_wco, torch.tensor(g_wco).to(device))
    return loss
