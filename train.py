...
def get_models(args, BERT_PT_PATH, trained=False, path_model_bert=None, path_model=None):
    ...
    n_conn_sql_dict = len(conn_sql_dict)
    ...
    model = Seq2SQL_v1(args.iS, args.hS, args.lS, args.dr, n_cond_ops, n_agg_ops, n_conn_sql_dict)
...
def get_data(path_wikisql, args):
    train_data, train_table, val_data, val_table, _, _ = load_wikisql(path_wikisql, args.toy_model, args.toy_size,
                                                                      no_w2i=True, no_hs_tok=True)
    train_loader, val_loader = get_loader_wikisql(train_data, val_data, args.bS, shuffle_train=True)

    return train_data, train_table, val_data, val_table, train_loader, val_loader
...
def train(train_loader, train_table, model, model_bert, opt, bert_config, tokenizer,
    ...
    for iB, t in enumerate(train_loader):
        ...
        g_sn, g_sc, g_sa, g_wn, g_wc, g_wo, g_wv, g_wco = get_g(sql_i)

        gt_cond_seq = []
        for _i in range(len(sql_i)):
            gt_cond_seq.append(sql_i[_i]['conds'])

        # get ground truth where-value index under CoreNLP tokenization scheme. It's done already on trainset.
        g_wvi_corenlp = get_g_wvi_corenlp(t, nlu_t, gt_cond_seq)
        ...
        # score
        s_sn, s_sc, s_sa, s_wn, s_wc, s_wo, s_wv, s_wco = model(wemb_n, l_n, wemb_h, l_hpu, l_hs, g_sn=g_sn,
                                                                g_sc=g_sc, g_sa=g_sa, g_wn=g_wn, g_wc=g_wc, g_wvi=g_wvi,
                                                                g_wco=g_wco)

        # Calculate loss & step
        loss = Loss_sw_se(s_sn, s_sc, s_sa, s_wn, s_wc, s_wo, s_wv, s_wco, g_sn, g_sc, g_sa, g_wn, g_wc, g_wo, g_wvi,
                          g_wco)
        ...
        cnt_wvi1_list, cnt_wv1_list, cnt_wco1_list = get_cnt_sw_list(g_sn, g_sc, g_sa, g_wn, g_wc, g_wo, g_wvi, g_wco,
                                                                     pr_sn,
        ...
        cnt_lx1_list = get_cnt_lx_list(cnt_sn1_list, cnt_sc1_list, cnt_sa1_list, cnt_wn1_list, cnt_wc1_list,
                                       cnt_wo1_list, cnt_wv1_list, cnt_wco1_list)
        # lx stands for logical form accuracy
        # there sn not update
        # Execution accuracy test.
        # cnt_x1_list, g_ans, pr_ans=0,0,0
        cnt_x1_list, g_ans, pr_ans = get_cnt_x_list(engine, tb, g_sc, g_sa, g_wco, sql_i, pr_sc, pr_sa, pr_wco,
                                                    pr_sql_i)
    ...
    acc = [ave_loss, acc_sn, acc_sc, acc_sa, acc_wn, acc_wc, acc_wo, acc_wvi, acc_wv, acc_wco, acc_lx, acc_x]
...
def test(data_loader, data_table, model, model_bert, bert_config, tokenizer,
    ...
    for iB, t in enumerate(data_loader):
        ...
        if not EG:
            ...
            # get loss & step
            loss = Loss_sw_se(s_sn, s_sc, s_sa, s_wn, s_wc, s_wo, s_wv, s_wco, g_sn, g_sc, g_sa, g_wn, g_wc, g_wo,
                              g_wvi, g_wco)
...
def print_result(epoch, acc, dname):
    ave_loss, acc_sn, acc_sc, acc_sa, acc_wn, acc_wc, acc_wo, acc_wvi, acc_wv, acc_wco, acc_lx, acc_x = acc

    print(f'{dname} results ------------')
    print(
        f" Epoch: {epoch}, ave loss: {ave_loss}, acc_sn: {acc_sn:.3f},acc_sc: {acc_sc:.3f}, acc_sa: {acc_sa:.3f}, acc_wn: {acc_wn:.3f}, \
        acc_wc: {acc_wc:.3f}, acc_wo: {acc_wo:.3f}, acc_wvi: {acc_wvi:.3f}, acc_wv: {acc_wv:.3f}, acc_wco: {acc_wco:.3f}, acc_lx: {acc_lx:.3f}, acc_x: {acc_x:.3f}"
    )
...
if __name__ == '__main__':
    ...
    path_save_for_evaluation = '../model'

    ## 3. Load data
    train_data, train_table, val_data, val_table, train_loader, val_loader = get_data(path_wikisql, args)
    ...
    for epoch in range(args.tepoch):
        ...
        acc_train, aux_out_train = train(train_loader,
                                         ...
                                         opt_bert=opt_bert,
                                         st_pos=0,
                                         path_db=path_wikisql+'/train',
                                         dset_name='train')

        # check DEV
        with torch.no_grad():
            acc_dev, results_dev, cnt_list = test(val_loader,
                                                  val_table,
                                                  model,
                                                  model_bert,
                                                  bert_config,
        ...
        acc_wc_t = acc_dev[5]
        if acc_wc_t > acc_lx_t_best:
            acc_lx_t_best = acc_wc_t
            epoch_best = epoch
            # save best model
            state = {'model': model.state_dict()}
        ...
        #     torch.save(state, os.path.join('../model', 'model_bert_best.pt'))
        #     print('saved')
        print(f" Best Dev wc acc: {acc_lx_t_best} at epoch: {epoch_best}")
